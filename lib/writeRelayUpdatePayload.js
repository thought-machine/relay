...
function mergeField(writer, fieldName, payload, operation) {
  ...
  require('fbjs/lib/invariant')(recordID, 'writeRelayUpdatePayload(): Expected a record ID in the response payload ' + 'supplied to update the store.');
...
function handleRangeDelete(writer, payload, config) {
  ...
  require('fbjs/lib/invariant')(recordID != null, 'writeRelayUpdatePayload(): Missing ID for deleted record at field `%s`.', config.deletedIDFieldName);
